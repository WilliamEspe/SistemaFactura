...
@section('content')
    <div class="max-w-7xl mx-auto py-8">
        <h2 class="text-2xl font-bold mb-4">Papelera de Clientes</h2>
                ...
                <tbody>
                    @forelse($clientes as $cliente)
                        <tr class="border-b">
                            ...
                            <td class="px-4 py-2">{{ $cliente->motivo_eliminacion }}</td>
                            <td class="px-4 py-2">
                                {{ $cliente->deleted_at->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}</td>
                            <td class="px-4 py-2">
                                <!-- Restaurar con motivo -->
                                <button onclick="openModal('restaurarModal-{{ $cliente->id }}')"
                                    class="text-green-600 hover:underline mr-2">Restaurar</button>

                                <!-- Eliminar definitivamente -->
                                <button onclick="openModal('eliminarDefinitivo-{{ $cliente->id }}')"
                                    class="text-red-600 hover:underline">Eliminar Definitivo</button>
                            </td>
                        </tr>

                        <!-- Modal Restaurar -->
                        <div id="restaurarModal-{{ $cliente->id }}"
                            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                            <div class="bg-white p-6 rounded shadow-md w-96">
                                <h3 class="text-lg font-bold mb-4">Restaurar Cliente</h3>
                                <form method="POST" action="{{ route('clientes.restaurar', $cliente->id) }}">
                                    @csrf
                                    ...
                                    <textarea name="motivo_restauracion" required class="w-full border rounded px-2 py-1 mb-4"
                                        rows="3" placeholder="Explica por qué se restaura el cliente..."></textarea>

                                    <div class="flex justify-end">
                                        <button type="button" onclick="closeModal('restaurarModal-{{ $cliente->id }}')"
                        ...
                        <div id="eliminarDefinitivo-{{ $cliente->id }}"
                            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                            <div class="bg-white p-6 rounded shadow-md w-96">
                                ...
                                <form method="POST" action="{{ route('clientes.eliminarDefinitivo', $cliente->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    ...
                                    <label class="block text-sm mb-2">
                                        <input type="checkbox" name="confirmacion" class="mr-1"
                                            onchange="document.getElementById('btnDef-{{ $cliente->id }}').disabled = !this.checked">
                                        Estoy consciente de que esta acción es irreversible.
                                    </label>

                                    <div class="flex justify-end mt-4">
                                        <button type="button" onclick="closeModal('eliminarDefinitivo-{{ $cliente->id }}')"
                                            class="mr-2 px-4 py-1">Cancelar</button>
                                        <button type="submit" id="btnDef-{{ $cliente->id }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-4 text-center text-gray-500">No hay clientes en papelera.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
